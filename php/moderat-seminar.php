<?php
if (isset($_POST['del_seminar'])) { // кнопка удаление книги
  ...
  if (isset($_SESSION['admin_good']) && $_SESSION['admin_good'] == 1) { // проверка является ли пользователь админом
    ...
    $events = 'Из каталога товаров был удален семинар '.$infa['title_seminar'].''; // подготовка текста события
    newevent($con_li, $today, $ip_address, $events); // передача данных в функцию

    $dell = $con_li->prepare( "DELETE FROM seminar WHERE id = ?" ); // подготавливаем наш запрос
  ...
  if (isset($_SESSION['admin_good']) && $_SESSION['admin_good'] == 1) { // проверка является ли пользователь админом
    ...
    $ip_address = $_SERVER['REMOTE_ADDR']; // получаем ip адрес
    $today = date("Y-m-d H:i:s"); // 2001-03-10 17:16:18 (the MySQL DATETIME format)
    $events = 'Администратор удалил отзыв пользователя '.$numphone_comment.' на семинар '.$infa['title_seminar'].''; // подготовка текста события
    newevent($con_li, $today, $ip_address, $events); // передача данных в функцию
    $razdel_book = 2;
